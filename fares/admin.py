...
class LegRuleAdmin(ImportExportModelAdmin):
    save_as = True
    list_display = ("id", "network", "from_area", "to_area",
                    "rider_category", "fare_container", "product", "leg_group")
    list_editable = ("network", "from_area", "to_area",
                     "rider_category", "fare_container", "product", "leg_group")
...
class CalendarAdmin(ImportExportModelAdmin):
    list_display = ("service_id", "monday", "tuesday", "wednesday", "thursday",
                    "friday", "saturday", "sunday", "start_date", "end_date")
    resource_classes = [CalendarResource]


class ProductAdmin(ImportExportModelAdmin):
    save_as = True
    list_display = ("id", "ref_id", "name", "amount",
                    "currency", "associated_rules_count")
...
class RiderAdmin(ImportExportModelAdmin):
    list_display = ("id", "ref_id", "name", "min_age",
                    "max_age", "eligibility_url", "notes",)
    list_editable = ("ref_id", "name", "min_age",
                     "max_age", "eligibility_url", "notes",)
...
class TransferRuleAdmin(ImportExportModelAdmin):
    save_as = True
    list_display = ("id", "from_leg_group", "to_leg_group", "fare_product",
                    "transfer_count", "duration_limit", "duration_limit_type", "fare_transfer_type")
    list_editable = ("from_leg_group", "to_leg_group", "fare_product", "transfer_count",
...
class RouteAdmin(ImportExportModelAdmin):
    list_display = ("route_id", "route_short_name",
                    "route_long_name", "network_id", "as_route")
    # list_editable = ("ref_id", )
